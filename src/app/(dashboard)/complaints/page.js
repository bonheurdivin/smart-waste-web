...
export default function ComplaintsPage() {
    ...
    const fetchComplaints = async () => {
        try {
            ...
            const response = await fetch('http://localhost/smart-waste-api/api/v1/complaints', { headers: { 'Authorization': `Bearer ${token}` } });
    ...
    const handleSave = async () => {
        ...
        try {
            const token = localStorage.getItem('admin_token');
            const response = await fetch(`http://localhost/smart-waste-api/api/v1/complaints/${selected.id}`, { method: 'PUT', headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' }, body: JSON.stringify(form) });
            const data = await response.json();
    ...
    const handleDelete = async (id) => {
        ...
        try {
            const token = localStorage.getItem('admin_token');
            const response = await fetch(`http://localhost/smart-waste-api/api/v1/complaints/${id}`, { method: 'DELETE', headers: { 'Authorization': `Bearer ${token}` } });
            const data = await response.json();
