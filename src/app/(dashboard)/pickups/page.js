...
export default function PickupsPage() {
    ...
    const fetchPickups = async () => {
        try {
            ...
            const response = await fetch('https://smart-waste-collector.up.railway.app/api/v1/pickups', { headers: { 'Authorization': `Bearer ${token}` } });
    ...
    const handleSave = async () => {
        ...
        try {
            const token = localStorage.getItem('admin_token');
            const url = editing ? `https://smart-waste-collector.up.railway.app/api/v1/pickups/${editing.id}` : 'https://smart-waste-collector.up.railway.app/api/v1/pickups';
