...
export default function PickupsPage() {
    ...
    const fetchPickups = async () => {
        try {
            ...
            const response = await fetch('http://localhost/smart-waste-api/api/v1/pickups', { headers: { 'Authorization': `Bearer ${token}` } });
    ...
    const handleSave = async () => {
        ...
        try {
            const token = localStorage.getItem('admin_token');
            const url = editing ? `http://localhost/smart-waste-api/api/v1/pickups/${editing.id}` : 'http://localhost/smart-waste-api/api/v1/pickups';
