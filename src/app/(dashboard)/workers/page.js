...
export default function WorkersPage() {
    ...
    const fetchUsers = async () => {
        try {
            const token = localStorage.getItem('admin_token');
            const response = await fetch('https://smart-waste-collector.up.railway.app/api/v1/users/workers', { headers: { 'Authorization': `Bearer ${token}` } });
            const data = await response.json();
    ...
    const handleSave = async () => {
        ...
        try {
            const token = localStorage.getItem('admin_token');
            const url = editing ? `https://smart-waste-collector.up.railway.app/api/v1/workers/${editing.id}` : 'https://smart-waste-collector.up.railway.app/api/v1/workers';
    ...
    const handleDelete = async (id) => {
        ...
        try {
            const token = localStorage.getItem('admin_token');
            const response = await fetch(`https://smart-waste-collector.up.railway.app/api/v1/workers/${id}`, { method: 'DELETE', headers: { 'Authorization': `Bearer ${token}` } });
            const data = await response.json();
