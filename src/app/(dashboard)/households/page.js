...
export default function HouseholdsPage() {
    ...
    const fetchPlans = async () => {
        try {
            const response = await fetch('https://smart-waste-collector.up.railway.app/api/v1/plans');
            const data = await response.json();
    ...
    const fetchResidents = async () => {
        try {
            const token = localStorage.getItem('admin_token');
            const response = await fetch('https://smart-waste-collector.up.railway.app/api/v1/users/residents', { headers: { 'Authorization': `Bearer ${token}` } });
            const data = await response.json();
    ...
    const handleSave = async () => {
        ...
        try {
            const token = localStorage.getItem('admin_token');
            const url = editing ? `https://smart-waste-collector.up.railway.app/api/v1/households/${editing.id}` : 'https://smart-waste-collector.up.railway.app/api/v1/households';
    ...
    const handleDelete = async (id) => {
        ...
        try {
            const token = localStorage.getItem('admin_token');
            const response = await fetch(`https://smart-waste-collector.up.railway.app/api/v1/households/${id}`, { method: 'DELETE', headers: { 'Authorization': `Bearer ${token}` } });
            const data = await response.json();
