...
export default function VehiclesPage() {
    ...
    const handleSave = async () => {
        ...
        try {
            const token = localStorage.getItem('admin_token');
            const url = editing ? `https://smartwaste.infinityfree.io/api/v1/vehicles/${editing.id}` : 'https://smartwaste.infinityfree.io/api/v1/vehicles';
    ...
    const handleDelete = async (id) => {
        ...
        try {
            const token = localStorage.getItem('admin_token');
            const response = await fetch(`https://smartwaste.infinityfree.io/api/v1/vehicles/${id}`, { method: 'DELETE', headers: { 'Authorization': `Bearer ${token}` } });
            const data = await response.json();
