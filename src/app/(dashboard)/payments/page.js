...
export default function PaymentsPage() {
    ...
    const fetchPayments = async () => {
        try {
            ...
            const response = await fetch('https://smart-waste-collector.up.railway.app/api/v1/payments', { headers: { 'Authorization': `Bearer ${token}` } });
    ...
    const handleSave = async () => {
        ...
        try {
            const token = localStorage.getItem('admin_token');
            const url = editing ? `https://smart-waste-collector.up.railway.app/api/v1/payments/${editing.id}` : 'https://smart-waste-collector.up.railway.app/api/v1/payments';
    ...
    const handleDelete = async (id) => {
        ...
        try {
            const token = localStorage.getItem('admin_token');
            const response = await fetch(`https://smart-waste-collector.up.railway.app/api/v1/payments/${id}`, { method: 'DELETE', headers: { 'Authorization': `Bearer ${token}` } });
            const data = await response.json();
